...
import AIMaturityTest from './tools/AIMaturityTest';

const IAPowers = () => {
  const [hoveredCard, setHoveredCard] = useState<number | null>(null);
  const [selectedTool, setSelectedTool] = useState<string | null>(null); 
  
  // El estado de las partículas se inicializa como un array vacío.
  ...
  // Llena el estado 'particles' con valores aleatorios, evitando el error de hidratación.
  useEffect(() => {
  ...
  return (
    <>
            ...
            {solutions.map((solution, index) => (
              ...
              >
                <div className="bg-gradient-to-br from-gray-800/80 to-gray-900/50 rounded-2xl border border-white/10 overflow-hidden backdrop-blur-sm transition-all duration-500 hover:border-cyan-400/30 hover:shadow-2xl hover:shadow-cyan-500/10">
                  <div className="relative min-h-[192px] bg-gradient-to-r from-cyan-900/20 to-purple-900/30 overflow-hidden">
                    <video
                      src={solution.video}
                      autoPlay
                      muted
      ...
      {/* Renderizado de Modales Interactivos */}
      <InteractiveModal
        isOpen={selectedTool === 'roi-calculator'}
      ...
      <InteractiveModal
        isOpen={selectedTool === 'readiness-assessment'}
        onClose={() => setSelectedTool(null)}
      >
        <AIMaturityTest />
      </InteractiveModal>
